...
function create_postgres_db {
 ...
 echo "PATH='$PATH:/usr/lib/postgresql/8.4/bin'" >> ~/.bashrc
 source ~/.bashrc
...
function create_geodjango_db {
 ...
 psql -d geodjango -f '/usr/share/postgresql/8.4/contrib/postgis-1.5/postgis.sql'
 psql -d geodjango -f '/usr/share/postgresql/8.4/contrib/postgis-1.5/spatial_ref_sys.sql'
}
...
function create_postgis_template_db {
 ...
 psql -d template_postgis -f '/usr/share/postgresql/8.4/contrib/postgis-1.5/postgis.sql' # Loading the PostGIS SQL routines
...
function main {
  install_package curl vim-gnome python-dev python-setuptools couchdb postgresql-8.4 postgresql-8.4-postgis libgeos-3.2.0 git-core python-gdal libpq-dev gettext libxml2-dev libxslt1-dev && \
