...
def delete_voided_datasenders(db_name):
    ...
    try:
        ...
        for datasender in dbm.database.query(map_voided_datasenders):
            try:
                user_profiles = NGOUserProfile.objects.filter(reporter_id=datasender)
                user = user_profiles[0].user
                if _is_only_datasender(user):
                    _delete_user_entry(dbm, user_profiles[0],logger)
            except ObjectDoesNotExist:
                logger.info("User with reporter-id: '%s' does not exist" % datasender)
    except Exception as e:
        logger.exception(e.message)


migrate(all_db_names(), delete_voided_datasenders, version=(10, 1, 4), threads=2)
