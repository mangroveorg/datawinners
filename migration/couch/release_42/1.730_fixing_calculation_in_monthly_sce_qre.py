...
function(doc) {
    ...
    function check_calculation(doc){
        var fields = ['service_counselling', 'service_ec', 'service_male_condom', 'service_pill', 'service_injectable',
                      'service_implant', 'service_iud', 'service_tl', 'service_vasectomy', 'service_STI',
                       'service_cacm', 'service_cacs'];
        var total = 0;
        for (var i=0; i< fields.length ; i++) {

            if (doc.values['section_service'][0][fields[i]] == 'NaN' ||
                doc.values['section_service'][0][fields[i]] == '' ||
                doc.values['section_service'][0][fields[i]] == null)
                continue;
            total += parseInt(doc.values['section_service'][0][fields[i]]);
        }
        return total != parseInt(doc.values['section_client2'][0]['service_totalfp']);
    }
