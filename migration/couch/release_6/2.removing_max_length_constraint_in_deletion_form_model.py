...
from migration.couch.release_6.utils import init_migrations, mark_start_of_migration, should_not_skip
...
def migrate_db(db, failed_managers):
    global manager
    try:
        ...
        try:
            ...
            log_statement("error:" + e.message, manager.database)
    except Exception as e:
...
def migrate_story_1924():
    ...
    for manager, exception_message in failed_managers:
        log_statement(" failed. the reason :  %s" % exception_message)
