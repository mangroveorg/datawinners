...
def create_subject_mapping(dbm, form_model):
    ...
    mapping = get_fields_mapping_by_field_def(doc_type=form_model.entity_type[0], fields_definition=fields_definition)
    es.put_mapping(dbm.database_name, form_model.entity_type[0], mapping)


def entity_search_update(entity_doc, dbm, bulk=False):
...
def contact_search_update(entity_doc, dbm, bulk=False):
    if entity_doc.aggregation_paths['_type'] == REPORTER_ENTITY_TYPE:
        return update_datasender_index(entity_doc, dbm, bulk=bulk)