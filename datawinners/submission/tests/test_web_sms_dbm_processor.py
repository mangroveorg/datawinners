...
from submission.request_processor import WebSMSDBMRequestProcessor, WebSMSTransportInfoRequestProcessor, SMSMessageRequestProcessor
...
class TestWebSMSRequestProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sms_message = "Hi"
        self.request = FakeRequest(post=dict(test_mode=True,message=self.sms_message), user=user)

    def test_should_put_dbm_in_request_for_web_sms_submission(self):
        processor = WebSMSDBMRequestProcessor()
        processor.process(self.request, self.mangrove_request)
    ...
    def test_should_put_message_in_request_for_web_sms_submission(self):
        ...
        processor.process(self.request, self.mangrove_request)
        self.assertEqual(self.sms_message, self.mangrove_request['message'])
