...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            logins_with_multiple_digests = PartialDigest.objects.values('login').annotate(dcount=Count('login')).filter(dcount__gte =2)
            for entry in logins_with_multiple_digests:
                multiple_digests = PartialDigest.objects.filter(login=entry['login'])
                for digest in multiple_digests:
                    digest.delete()
