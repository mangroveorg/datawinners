...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for org in orm.OrganizationSetting.objects.all():
            if org.smsc is not None and not is_empty(org.sms_tel_number):
                outgoing_obj = orm.OutgoingNumberSetting.objects.create(phone_number=org.sms_tel_number.split(',')[0].strip(),smsc=org.smsc)
                outgoing_obj.save()
    ...
    def backwards(self, orm):
        for org in orm.OrganizationSetting.objects.all():
            if org.outgoing_number is not None:
                org.outgoing_number.delete()
                org.outgoing_number = None
                org.save()
