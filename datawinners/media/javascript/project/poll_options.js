    ...
    self.activate_poll = function() {
        ...
        $.post(activate_poll_url, data).done(function (response) {
            ...
            if (responseJson['success']) {
                ...
                $('<div class="information_box">' + responseJson['message'] + '<a id="active_poll_name" class="link_color" href="/project/poll/' + responseJson['question_id_active'] + '">' + responseJson['question_name_active'] + '</a></div>').insertBefore($("#poll_success"))
                $('.message-box').delay(2000).fadeOut();
            }
        });
