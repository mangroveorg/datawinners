...
//                keyup:    onEditorKeyUp,
        ...
        function checkForTrigger(look_ahead) {
            ...
            var selectionStartFix = $.browser.webkit ? 0 : -1,
                sStart = getInputSelection(elEditor[0]).start + selectionStartFix,
        ...
        function is_nav_key(key){
            var nav_keys = [KEY.UP, KEY.DOWN, KEY.LEFT, KEY.RIGHT, KEY.HOME, KEY.END, KEY.TAB];
            return nav_keys.indexOf(key) != -1
        }
...
//                sEnd = elEditor[0].selectionEnd,
            ...
            if(sEnd >= sStart && !is_nav_key(editorKeyCode)) {
                for(var i=0; i<tagsCollection.length; i++){
                    var tag = tagsCollection[i];

                    var tagStart = tag[0] ;
                    var tagEnd = tagStart + tag[1];
                    var cursorStart = sStart;
                    var cursorEnd = sEnd;
                    if(editorKeyCode == keys['DELETE']) cursorEnd +=1;
                    if(editorKeyCode == keys.BACKSPACE) cursorStart--;
                    if (cursorStart<=tagEnd && cursorEnd>=tagStart) {
                        e.preventDefault();
                        return
                    }
                }
            ...
            switch (e.keyCode) {
                ...
                case keys['DELETE']:
                    ...
                    if(e.keyCode == keys['DELETE'] && sStart == sEnd && sStart >= 0){
                        sEnd += 1;
                    }
                    if(sEnd > sStart) {
...
//                    break;
        ...
        function onEditorKeyUp (e) {
            if (editorInPasteMode) {
                ...
                var sStart = getInputSelection(elEditor[0]).start,
                    sEnd = getInputSelection(elEditor[0]).end;
        ...
        function onEditorInputIe8(e){
            var timer;
            if ($.browser.msie && $.browser.version <= 9){
                if([17,18,91].indexOf(editorKeyCode) != -1){
                    return false;
                }
                timer = setTimeout(onEditorInput(e), 500);
                clearTimeout(timer);
            }
        }
        ...
        function onEditorInput (e) {
            ...
            if (editorKeyCode != KEY.BACKSPACE && editorKeyCode != KEY['DELETE']) {
                if (editorSelectionLength > 0) {
                    ...
                    var sStart = getInputSelection(elEditor[0]).start + selectionStartFix,
                    ...
                    shiftTagsPosition(sStart, tags_shift_positions);
                } else if (!editorInPasteMode) {
                    // char input - shift with 1
                    var sStart = getInputSelection(elEditor[0]).start + selectionStartFix,
                        sEnd = getInputSelection(elEditor[0]).end + selectionStartFix,
        ...
        function shiftTagsPosition (afterPosition, position_shift) {
            tagsCollection = _.map(tagsCollection, function (tagPos) {
                if (tagPos[0] + tagPos[1] >= afterPosition) {
