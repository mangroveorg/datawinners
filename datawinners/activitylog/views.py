        ...
        for key, value in filter.items():
            if value != "":
                if key == "daterange":
                    ...
                    args["log_date__gte"] = convert_to_ymd(convert_local_to_utc(dates[0] + " 00:00:00", time_delta))
                    try:
                        end_date = date.today()
                        if len(dates) > 1:
                            # end_date = convert_dmy_to_ymd(dates[1])
                            end_date = convert_to_ymd(convert_local_to_utc(dates[1] + " 23:59:59", time_delta))
                        else:
                            end_date = convert_to_ymd(convert_local_to_utc(dates[0] + " 23:59:59", time_delta))
                    except KeyError:
                        pass
