...
def get_filter_values(dbm, config, filters):
    ...
    if config.date_filter:
        ...
        combination_keys = filter_values and _combine_keys(combination_keys, keys) or _combine_keys(filter_value and not keys and [[]] or [], keys)
...
def _filter_value(qn, filters):
    ...
    if _type(qn, filters) == "date":
        ...
        value = value and len(value) == 1 and value + value or value
