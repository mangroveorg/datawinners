...
def _get_map_function(questionnaire_ids_string, combined_view_key):
    return "function(doc) {if(doc.document_type == 'SurveyResponse' && [%s].indexOf(doc.form_model_id) > -1) {var keys = [%s];for(key in keys) {var date = keys[key].split('.');if(date.length == 3) {var tmp = date[0]; date[0] = date[1]; date[1] = tmp;date = Date.parse(date.join('/'));if(date) {keys[key] = date;}}}emit(keys, 1);}}" % (questionnaire_ids_string, combined_view_key)
