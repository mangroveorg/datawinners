...
from datawinners.blue.xform_bridge import XlsFormParser, UppercaseNamesNotSupportedException, \
    NestedRepeatsNotSupportedException, MultipleLanguagesNotSupportedException, get_generated_xform_id_name, LabelForChoiceNotPresentException
...
class TestXformBridge(unittest.TestCase):
    ...
    def test_should_raise_exception_when_label_defined_in_single_explict_language(self):
        with patch('datawinners.blue.xform_bridge.parse_file_to_json') as get_xform_dict:
            xls_form_parser = XlsFormParser('some_path', 'questionnaire_name')
            fields = {'children': [{u'bind': {u'required': u'yes'}, u'type': u'text', u'name': u'name',
    ...
    def test_should_raise_exception_when_hint_defined_in_one_or_more_languages(self):
        with patch('datawinners.blue.xform_bridge.parse_file_to_json') as get_xform_dict:
            xls_form_parser = XlsFormParser('some_path', 'questionnaire_name')
            fields = {'children': [{u'bind': {u'required': u'yes'}, u'type': u'text', u'name': u'name',
    ...
    def test_should_raise_exception_when_the_label_in_choice_sheets_has_language(self):
        with patch('datawinners.blue.xform_bridge.parse_file_to_json') as get_xform_dict:
            xls_form_parser = XlsFormParser('some_path', 'questionnaire_name')
            fields = {'children': [{u'bind': {u'required': u'yes'}, u'type': u'text', u'name': u'name',
