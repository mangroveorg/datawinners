...
class TestAdvancedQuestionnaireSubmissionExporter(unittest.TestCase):
    def test_should_create_headers_(self):
        ...
        columns = OrderedDict([('ds_id', {'label': 'Data Sender Id'}),
                               ('uuid1_family',
                                {'fields': family_dict, 'type': 'field_set', 'code': 'field_code', 'label': 'Family',
        ...
        preferences = [dict(data='ds_id', visibility='True', title='Data Sender Id'),
                       dict(data='uuid1_family', visibility='True', children=[dict(data='name', visibility='True',title='Name'),
                                                                              dict(data='age', visibility='True',title='Age')]),
                       dict(data='uuid1_city', visibility=False, title='City'), dict(data='area', visibility='True', title='Area')]

        headers = AdvancedQuestionnaireSubmissionExportHeaderCreator(columns, form_model_mock,
                                                                     preferences).create_headers(False)
    ...
    def test_should_create_header_geocode_field(self):
        ...
        preferences = [dict(data='uuid1_city', title='G P S Coordinates', visibility=True),
                       dict(data='area', title='Area', visibility=False)]
