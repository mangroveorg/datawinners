...
class Rule(object):
    ...
    def update_xform(self, old_questionnaire, new_questionnaire):
        ...
        self._update_xform(new_questionnaire.fields, old_questionnaire.fields, xform.get_body_node())

        old_questionnaire.xform = '<?xml version="1.0" encoding="utf-8"?>%s' % ET.tostring(xform.root_node)

    def _update_xform(self, new_fields, old_fields, parent_node):
        for old_field in old_fields:
            ...
            if new_field and not is_calculated_field:
                if isinstance(old_field, FieldSet):
                    self._update_xform(new_field[0].fields, old_field.fields, node)
