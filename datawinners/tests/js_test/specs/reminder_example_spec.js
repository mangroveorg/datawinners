describe("Calculation for next deadline for reminders", function() {
    ...
    it("should update the next deadline example when month is changed to week", function () {
        ...
        var expected_date_for_month = new Date(2014,11,6);
        ...
        var expected_date_for_week = new Date(2014,11,5);
    ...
    it("should update the after deadline example when month is selected", function () {
        sinon.useFakeTimers(new Date(2014,11,4).getTime());

        var reminder_view_model = new ReminderSettingsModel();

        var reminderAfterDeadline = reminder_view_model.reminder_after_deadline;
        reminderAfterDeadline.multiplier = 1;
        reminder_view_model.selected_frequency('month');
        reminder_view_model.select_day(4);
        reminderAfterDeadline.enable(true);
        reminderAfterDeadline.number_of_days(2);

        expect(reminder_view_model.next_deadline_string()).toEqual("Next deadline: 4 January 2015");
        expect(reminder_view_model.reminder_after_deadline.next_date_as_string()).toEqual("Next scheduled Reminder: 6 December 2014");

    });

    it("should update the after deadline example when week is selected", function () {
        sinon.useFakeTimers(new Date(2014,11,4).getTime());
        ...
        expect(reminder_view_model.next_deadline_string()).toEqual("Next deadline: Friday, 5 December 2014");
        expect(reminder_view_model.reminder_after_deadline.next_date_as_string()).toEqual("Next scheduled Reminder: Sunday, 7 December 2014");
