...
class TestExport(MangroveTestCase):
    ...
    def test_should_get_field_instructions_for_header_in_excel_export(self):
        ...
        self.assertEqual(['name', 'short_code', 'location', 'geo_code', 'mobile_number', 'email'], fields)
        self.assertEqual(
            ["What is the subject's name?", "What is the subject's Unique ID Number", "What is the subject's location?",
             "What is the subject's GPS co-ordinates?", 'What is the mobile number associated with the subject?',
             "What is the subject's email"], header)
        self.assertEqual(['n', 's', 'l', 'g', 'm', 'email'], codes)