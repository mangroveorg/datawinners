...
class SubmissionAnalyzer(object):
    def __init__(self, form_model, manager, org_id, submissions, keyword=None, is_for_submission_page=False):
        ...
        self._data_senders = []
        self.data_sender_helper = DataSenderHelper(self.manager, self.form_model.form_code)
        self._subject_list = []
    ...
    def _get_leading_part_for_submission_page(self, submission):
        ...
        return filter(lambda x: x, [submission.id, data_sender, submission_date, status, error_message, subject, rp])
