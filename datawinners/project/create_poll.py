...
def _associate_data_senders_to_poll(manager, questionnaire, selected_option):
    data_senders_list = []
...
def _create_text_question(manager, questionnaire, question):
...
def _save_poll(manager, questionnaire, selected_option, question):
    _create_text_question(manager, questionnaire, question)
    _associate_data_senders_to_poll(manager, questionnaire, selected_option)
...
def _construct_questionnaire(request):
    ...
    poll_questionnaire_code = 'poll_' + str(time.time()).split('.')[0]
    questionnaire_code = poll_questionnaire_code
...
def _check_if_smsc_is_configured(organization):
...
def create_poll(request):
    if request.method == 'POST':
        ...
        smsc = _check_if_smsc_is_configured(organization)
        if smsc is None:
        ...
        manager = get_database_manager(request.user)
        is_active, active_project_id, active_project_name = get_active_form_model_name_and_id(manager)
        if not is_active:
            ...
            if not project_name_unique:
                ...
                return HttpResponse(
                    json.dumps({'success': True, 'active_project_id': questionnaire.id, 'project_code': questionnaire.form_code}))
            return HttpResponse(json.dumps({'success': False,
                                            'error_message': error_message,
                                            'project_name_unique': project_name_unique}))

        else:
            message = _("To activate the Poll you must first deactivate your current %s. You may only have one active Poll at a time.") % active_project_name
            return HttpResponse(json.dumps({'success': False,
                                    'error_message': message,
                                    'question_id_active': active_project_id,
                                    'question_name_active': active_project_name}))
