...
from datawinners.project.submission.export import export_filename, create_zipped_excel_response, create_zipped_excel_file
...
class SubmissionExporter:
    ...
    def get_file(self, columns, submission_list, submission_type):
        header_list, formatted_values = SubmissionFormatter(columns, self.local_time_delta).format_tabular_data(
            submission_list)
        return create_zipped_excel_file(header_list, formatted_values,
                                            export_filename(submission_type, self.project_name))
    ...
    def get_export_file(self, submission_type, query_params):
        columns = SubmissionExcelHeader(self.form_model, submission_type, self.language).get_columns()

        search_results, query_fields = get_scrolling_submissions_query(self.dbm, self.form_model, query_params,
                                                                       self.local_time_delta)
        return self.get_file(columns, search_results, submission_type)
