...
def export_sms_details_to_excel(modeladmin,request,query_set):
    ...
    for sms in query_set:
        details = []
        details.append(sms.organization_id)
        details.append(sms.status)
        delivered_date_time = ExcelDate(datetime.combine(sms.delivered_at, datetime.min.time()),'dd.mm.yyyy') if sms.delivered_at else None
        details.append(delivered_date_time)
        details.append(sms.msg_from)
        details.append(sms.msg_to)
        details.append(sms.message)
        list.append(details)
