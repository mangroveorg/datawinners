...
def submit_to_player(incoming_request):
    try:
        ...
        response = sms_player.accept(incoming_request['transport_info'], incoming_request['form_model'].form_code, incoming_request['values'])
        ...
        DatawinnerLog(message=_message, from_number=_from, to_number=_to, form_code=exception.data[0],error=message).save()
    except NumberNotRegisteredException as exception:
        message = get_exception_message_for(exception=exception, channel=SMS)
        DatawinnerLog(message=_message, from_number=_from, to_number=_to, error=message).save()
...
def find_parser(incoming_request):
    sms_parser = SMSParser() if settings.USE_ORDERED_SMS_PARSER else OrderSMSParser(incoming_request['dbm'])
    try:
...
def find_dbm(request):
    ...
    if _to is None:
        incoming_request['outgoing_message'] = "Your organization does not have a telephone number assigned. Please contact DataWinners Support."
        return incoming_request
    ...
    try:
        ...
        message = get_exception_message_for(exception=exception, channel=SMS)
        incoming_request['outgoing_message'] = message
        DatawinnerLog(message=incoming_request['incoming_message'], from_number=_from, to_number=_to, error=message).save()
        return incoming_request
